...
const fieldName = 'field_prename'; // The expected field name in the API
...
const displayLabel = computed(() => t('form.first_name') || config.value?.label || 'First Name');
const displayHelp = computed(() => t('form.first_name_description') || config.value?.description || null);
const displayPlaceholder = computed(() => t('form.first_name_placeholder') || config.value?.widget_settings?.placeholder || 'Enter first name');
...
const validate = (value: string) => {
    ...
    if (!isValid && hasUserInteracted.value) {
        error.value = t('validation.first_name_required') ||
